...
void Generator::bfs_layer(const std::vector<std::vector<CircuitSeq *>> &dags,
                          int max_num_param_gates, Dataset &dataset,
                          std::vector<CircuitSeq *> *new_representatives,
                          bool invoke_python_verifier,
...
void Generator::findCombinations(std::set<int>::iterator it,
  ...
  // combination containing this element
  combination.push_back(*it);
  findCombinations(std::next(it), end, n - 1, combination, result);

  // combination not containing this element
  combination.pop_back();
  findCombinations(std::next(it), end, n, combination, result);
}
...
std::vector<std::vector<int>> Generator::get_combinations(std::set<int> &s,
  ...
  std::vector<std::vector<int>> result;
  std::vector<int> combination;
  findCombinations(s.begin(), s.end(), n, combination, result);
  return result;
}
